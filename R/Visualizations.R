...
#' @param compNames A character vector of comparison names. "all" selects all comparisons.
...
plotPvalueDistr <- function(fulldata, compNames="all",
  ...
  PValue <- as.matrix(rdat[,grep("^p_values_", colnames(rdat)),drop=F])
  # get only the p-values for the tests and comparisons
  PValue <- PValue[, grep(paste(paste0("^p_values_", testNames2), collapse = "|"), colnames(PValue)),drop=F]
  PValue <- PValue[, grep(paste(paste0(compNames, "$"), collapse = "|"), colnames(PValue)),drop=F]
...
plotVolcano <- function(fulldata, compNames = "all",
  ...
  Qvalue <- as.matrix(rdat[,grep("^FDR_", colnames(rdat)),drop=F])
  LogRatios <- as.matrix(rdat[,grep("^log_ratios_", colnames(rdat)),drop=F])
  # get only the p-values for the tests and comparisons
  Qvalue <- Qvalue[, grep(paste(paste0("^FDR_", testNames2), collapse = "|"), colnames(Qvalue)), drop=F]
  LogRatios <- LogRatios[, grep(paste(paste0("^log_ratios_", compNames), collapse = "|"), colnames(LogRatios)), drop=F]
  Qvalue <- Qvalue[, grep(paste(paste0(compNames, "$"), collapse = "|"), colnames(Qvalue)), drop=F]
...
plotExpression <- function(fulldata, compNames = "all",
  ...
  if (ncol(Qvalue) > 0 & length(sel_prots) > 1) {
    ...
    SubSetLR <- SubSetLR[order(rowMins(SubSetQval[,seq_len(NumComps),drop=F],na.rm=T)),,drop=F]

    SubSet <- SubSetLR[seq_len(min(nrow(SubSetLR),30)),,drop=F]
    indices <- rownames(SubSet)
    tdat <- as.matrix(dat[rownames(SubSet), (rep(seq_len(NumReps),NumCond)-1)*NumCond+rep(seq_len(NumCond),each=NumReps),drop=F])
    rownames(tdat) <- strtrim(rownames(tdat), 20)
    MeanSet <- SDSet <-  matrix(NA,nrow=nrow(tdat),ncol=NumCond,dimnames =
                                  list(x=rownames(tdat),y=paste("Condition",seq_len(NumCond))))
    for (c in 1:NumCond) {
      MeanSet[,c] <- rowMeans(tdat[,seq_len(NumReps) + (c-1)*NumReps,drop=F],na.rm=T)

      SDSet[,c] <- rowSds(tdat[,seq_len(NumReps) + (c-1)*NumReps,drop=F],na.rm=T)
    }
    if (profiles_scale) {
    ...
    if (nrow(MeanSet)>1) {
      for (i in 2:nrow(MeanSet)){
        suppressWarnings(plotCI(seq_len(NumCond)+runif(1,-0.1,0.1),MeanSet[i,],
    ...
    if (length(SubSet)> 0) {
      ...
      for (t in 1:NumTests) {
        ...
        tsign <- FCRegs[indices,(t-1)*(NumComps)+(seq_len(NumComps)),drop=F]<qlim
      ...
      mtext(paste("Successful statistical tests\nfor threshold given above.\nFrom outer to inner circles",
                  paste("Track ",seq_len(NumTests),": ",testNames,sep="",collapse="\n"),sep="\n"),
...
#' using an UpSet plot. Summarizes all comparisons from all tests
#'
#' @param fulldata A `SummarizedExperiment` object containing the data.
...
plotUpset <- function (fulldata, qlim=0.05, fclim=c(0,0)) {
  ...
  # check compnames and testnames with metadata
  if (is.null(metadata(fulldata)$compNames) | is.null(metadata(fulldata)$testNames)) {
stop("No metadata for statistical testing  found in the SummarizedExperiment object. Please run the
     statistical tests")
  }
  ...
  if (!is.null(FCRegs)) {
    ...
    if(length(WhereRegs)>0) {
      ...
      UpSetR::upset(as.data.frame(WhereRegs),nsets=ncol(WhereRegs),mainbar.y.label = "Significant features",
                    ...
                    set.metadata = list(data = data.frame(set=colnames(WhereRegs),cols=tcolnames,crab=seq_len(ncol(WhereRegs))),
...
plotRegNumber <- function( fulldata, compNames = "all",
  ...
  if (!is.null(FCRegs)) {
    ...
    for (i in seq_len(NumComps)) {
      plotData <- vapply(tmpX, function(x) sum(FCRegs[, i] < x, na.rm = TRUE), numeric(1))
      plotData[plotData == 0] <- NA

      plot(tmpX, plotData, main = paste("Comparison", i), xlab = "FDR threshold",
      ...
      for (j in seq_len(NumTests - 1)) {
        count_num <- vapply(tmpX, function(x) sum(FCRegs[, (NumComps) * j + i] < x, na.rm = TRUE), numeric(1))
...
#'
#' @export
plotHeatmaply <- function( fulldata, sel_prots="all", heatmap_scale="none", file=NULL, ...) {
  ...
  if (ncol(Qvalue) > 0 & length(sel_prots) > 0) {
    ...
    if (!is.null(SubSetLR)) {
      if (length(SubSetLR)> 0 & nrow(SubSetLR)>1) {
        ...
        tdat <- dat[rownames(SubSetLR), (rep(seq_len(NumReps),NumCond)-1)*NumCond+rep(seq_len(NumCond),each=NumReps),drop=F]
        ...
        tqvals <- Qvalue[rownames(SubSetLR),seq_len(NumComps),drop=F]
